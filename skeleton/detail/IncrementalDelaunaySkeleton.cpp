...
Point circumcenter(const Point& a, const Point& b, const Point& c) {
    ...
    const double aa = common::squaredLength(a);
    const double bb = common::squaredLength(b);
    const double cc = common::squaredLength(c);
    return {(aa * (b.y() - c.y()) + bb * (c.y() - a.y()) + cc * (a.y() - b.y())) / divisor,
            (aa * (c.x() - b.x()) + bb * (a.y() - c.y()) + cc * (b.x() - a.x())) / divisor};
}

Point closestPointOnSegment(const Point& point, const Point& first, const Point& second) {
    const Point direction = second - first;
    const double squared_length = common::squaredLength(direction);
    if (squared_length <= kEpsilon) return first;
    const double projection = std::clamp(
        common::dot(point - first, direction) / squared_length, 0.0, 1.0);
    return first + direction * projection;
}

Point fallbackInteriorPoint(const std::vector<Point>& polygon) {
    for (std::size_t i = 0; i < polygon.size(); ++i) {
        ...
        const Point candidate = (previous + current + next) / 3.0;
        if (contains(polygon, candidate)) return candidate;
    }
    throw std::domain_error("Unable to determine an interior point for the polygon");
}
...
bool inCircle(const Triangle& triangle, const std::vector<Point>& points, const Point& p) {
    ...
    // Treat cocircular points as inside so boundary samples on regular edges
    // replace stale triangles instead of leaving an invalid triangulation.
...
std::vector<Triangle> triangulate(std::vector<Point> points) {
    ...
    for (std::size_t p = 0; p < count; ++p) {
        ...
        for (const auto& t : triangles) {
            if (!inCircle(t, points, points[p])) {
                kept.push_back(t);
                continue;
            }
            for (std::size_t e = 0; e < 3; ++e)
                ++edges[std::minmax(t.vertices[e], t.vertices[(e + 1) % 3])];
        }
    ...
    triangles.erase(std::remove_if(triangles.begin(), triangles.end(), [count](const Triangle& t) {
        return std::any_of(t.vertices.begin(), t.vertices.end(), [count](std::size_t v) {
            return v >= count;
        });
    }), triangles.end());
    return triangles;
...
std::vector<IncrementalDelaunayEdge> medialAxis(
    ...
    if (options.max_boundary_segment_length < 0.0)
        throw std::invalid_argument("The boundary sampling length cannot be negative");
    validate(polygon);
    ...
    const double spacing = options.max_boundary_segment_length > 0
                               ? options.max_boundary_segment_length
                               : std::hypot(max_x - min_x, max_y - min_y) / 64.0;
    ...
    for (std::size_t i = 0; i < polygon.size(); ++i) {
        ...
        for (std::size_t j = 0; j < n; ++j) {
            samples.push_back(a + (b - a) * (static_cast<double>(j) / n));
        }
    }
    const auto triangles = triangulate(samples);
    std::map<std::pair<std::size_t, std::size_t>, std::size_t> adjacent;
    std::vector<IncrementalDelaunayEdge> result;
    for (std::size_t i = 0; i < triangles.size(); ++i) {
        for (std::size_t e = 0; e < 3; ++e) {
            const auto edge = std::minmax(triangles[i].vertices[e],
                                          triangles[i].vertices[(e + 1) % 3]);
            const auto [it, new_edge] = adjacent.emplace(edge, i);
            if (!new_edge) {
                if (contains(polygon, triangles[i].center) &&
                    contains(polygon, triangles[it->second].center))
                    result.push_back({triangles[it->second].center, triangles[i].center});
            }
        }
    }
    if (result.empty()) {
        const Point interior = detail::fallbackInteriorPoint(polygon);
        Point nearest = polygon.front();
        double nearest_distance = std::numeric_limits<double>::infinity();
        for (std::size_t i = 0; i < polygon.size(); ++i) {
            const Point boundary = detail::closestPointOnSegment(
                interior, polygon[i], polygon[(i + 1) % polygon.size()]);
            const double distance = common::squaredDistance(interior, boundary);
            if (distance < nearest_distance) {
                nearest_distance = distance;
                nearest = boundary;
            }
        }
        result.push_back({interior, nearest});
    }
    return result;
}
