...
TEST(FortuneVoronoiTest, SplitsBoundsBetweenTwoSites) {
    const std::vector<common::geometry::BasePoint<int>> sites{{2, 5}, {8, 5}};

    const auto diagram = voronoiDiagram(sites, common::geometry::Aabb<int>{0, 0, 10, 10});
    const auto& cells = diagram.cells;

    ASSERT_EQ(cells.size(), 2U);
    ASSERT_EQ(diagram.edges.size(), 1U);
    EXPECT_EQ(halfPlaneVoronoiCells(sites, common::geometry::Aabb<int>{0, 0, 10, 10}).size(),
              cells.size());
...
TEST(FortuneVoronoiTest, ProducesBoundedCellsAndEdgesForTriangle) {
    const std::vector<common::geometry::BasePoint<float>> sites{
        {2.0F, 2.0F}, {8.0F, 2.0F}, {5.0F, 8.0F}};
    const common::geometry::Aabb<int> bounds{0, 0, 10, 10};
    ...
    for (const auto& cell : diagram.cells) {
        ...
        for (const auto& vertex : cell.vertices) {
            const double cell_distance = common::geometryutil::squaredDistance(cell.site, vertex);
            for (const auto& site : sites) {
                EXPECT_LE(cell_distance,
                          common::geometryutil::squaredDistance(
...
TEST(FortuneVoronoiTest, HandlesCollinearSitesWithoutUnboundedOutput) {
    const std::vector<common::geometry::BasePoint<int>> sites{{2, 5}, {6, 5}, {10, 5}};

    const auto diagram = voronoiDiagram(sites, common::geometry::Aabb<int>{0, 0, 12, 10});
...
TEST(FortuneVoronoiTest, HandlesCocircularSitesWithoutZeroLengthDiagonals) {
    const std::vector<common::geometry::BasePoint<int>> sites{
        {2, 2}, {8, 2}, {8, 8}, {2, 8}};
...
TEST(FortuneVoronoiTest, ProcessesSitesSharingSweepLevels) {
    const std::vector<common::geometry::BasePoint<double>> sites{
        {1.0, 9.0}, {5.0, 9.0}, {9.0, 9.0},
        {2.0, 2.0}, {8.0, 2.0}, {5.0, 5.0}};

    const auto diagram =
        voronoiDiagram(sites, common::geometry::Aabb<double>{0.0, 0.0, 10.0, 10.0});
    ...
    for (const auto& edge : diagram.edges) {
        for (const Point& endpoint : {edge.start, edge.end}) {
            const double edge_distance =
                common::geometryutil::squaredDistance(edge.firstSite, endpoint);
            EXPECT_NEAR(edge_distance,
                        common::geometryutil::squaredDistance(edge.secondSite, endpoint), 1e-8);
            for (const auto& site : sites) {
                EXPECT_LE(edge_distance,
                          common::geometryutil::squaredDistance(Point{site.x(), site.y()}, endpoint) +
                              1e-8);
            }
        }
    }
...
TEST(FortuneVoronoiTest, BuildsManyNondegenerateSites) {
    std::vector<common::geometry::BasePoint<double>> sites;
    sites.reserve(128);
    ...
    const auto diagram =
        voronoiDiagram(sites, common::geometry::Aabb<double>{0.0, 0.0, 1000.0, 1000.0});
    ...
    for (const auto& edge : diagram.edges) {
        EXPECT_NEAR(common::geometryutil::squaredDistance(edge.firstSite, edge.start),
                    common::geometryutil::squaredDistance(edge.secondSite, edge.start), 1e-5);
        EXPECT_NEAR(common::geometryutil::squaredDistance(edge.firstSite, edge.end),
                    common::geometryutil::squaredDistance(edge.secondSite, edge.end), 1e-5);
        for (const auto& site : sites) {
            const Point double_site{site.x(), site.y()};
            EXPECT_LE(common::geometryutil::squaredDistance(edge.firstSite, edge.start),
                      common::geometryutil::squaredDistance(double_site, edge.start) + 1e-4);
            EXPECT_LE(common::geometryutil::squaredDistance(edge.firstSite, edge.end),
                      common::geometryutil::squaredDistance(double_site, edge.end) + 1e-4);
        }
    }
}

TEST(FortuneVoronoiTest, RejectsDuplicateSites) {
    const std::vector<common::geometry::BasePoint<double>> sites{{1.0, 1.0}, {1.0, 1.0}};

    EXPECT_THROW(voronoiDiagram(sites, common::geometry::Aabb<double>{0.0, 0.0, 2.0, 2.0}),
