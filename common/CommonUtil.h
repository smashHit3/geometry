...
constexpr bool almost_equal(
    ...
    value_type tolerance = static_cast<value_type>(default_tolerance)) {
    if (tolerance < 0) {
...
template <typename value_type,
          std::enable_if_t<std::is_arithmetic_v<value_type>, int> = 0>
auto round_to_precision(value_type value, int decimal_places)
