...
// Binary bounding-volume hierarchy with leaf entry ranges.
struct Point {
    long long x;
    long long y;
};
...
int build(int begin, int end) {
    ...
    nodes.push_back(Node{box, begin, end});
    ...
    while (!pending.empty()) {
        ...
        if (node.leaf) {
            for (int i = node.begin; i < node.end; ++i) {
                const int candidate = pointIndices[i];
        ...
        const Distance leftDistance = squaredDistanceToBox(points[queryIndex], nodes[node.left].box);
        const Distance rightDistance =
            squaredDistanceToBox(points[queryIndex], nodes[node.right].box);
